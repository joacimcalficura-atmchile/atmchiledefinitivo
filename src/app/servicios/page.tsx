        ...
        <div className="flex flex-col items-center max-w-sm w-full px-6">
            <div className="relative w-56 h-56 flex items-center justify-center mb-10">
                ...
                <m.span animate={{ y: [-4, 4, -4] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
                    ...
                    <span className="text-slate-400 font-light">{"<"}</span> ATM <span className="text-slate-400 font-light">{"/>"}</span>
                </m.span>
            </div>
            <p className="text-[10px] font-black tracking-[0.35em] text-[#0047AB] uppercase mb-5">Iniciando Transformación...</p>
            <div className="w-full h-[2px] bg-slate-200/50 rounded-full overflow-hidden">
                <m.div initial={{ width: "0%" }} animate={{ width: "100%" }} transition={{ duration: 2.6, ease: smoothEase }} className="h-full bg-gradient-to-r from-[#0047AB] to-[#22D3EE]" />
            </div>
        </div>
    </m.div>
);
...
export default function ServiciosPage() {
    ...
    useEffect(() => {
        const t = setTimeout(() => setIsLoading(false), 2000); // Reducido para mejor UX
